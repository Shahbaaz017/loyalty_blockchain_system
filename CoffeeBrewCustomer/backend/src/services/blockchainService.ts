...
import CoffeeCoinABI from '../config/CoffeeCoin.json'; // Ensure this path is correct
...
const coffeeCoinContract = new Contract(coffeeCoinContractAddress!, CoffeeCoinABI.abi, provider);
const contractInterface = new Interface(CoffeeCoinABI.abi);
...
let MINT_FUNCTION_SIG_HASH: string | null = null;
let SPECIFIC_BURN_FUNCTION_SIG_HASH: string | null = null;
...
try {
  ...
  console.warn(`[Service] Error deriving MINT_FUNCTION_SIG_HASH for '${MINT_FUNCTION_SIG}'. Error: ${(e as Error).message}`);
}
...
try {
  ...
  console.warn(`[Service] Error deriving SPECIFIC_BURN_FUNCTION_SIG_HASH for '${SPECIFIC_BURN_FUNCTION_SIG}'. Error: ${(e as Error).message}`);
}


// --- STANDARD USER-FACING BLOCKCHAIN INTERACTION FUNCTIONS (using RPC) ---
export const getTokenName = async (): Promise<string> => {
  try { return await coffeeCoinContract.name(); }
  catch (error) { console.error('[Service] Error fetching token name (RPC):', error); throw error; }
};

export const getTokenSymbol = async (): Promise<string> => {
  try { return await coffeeCoinContract.symbol(); }
  catch (error) { console.error('[Service] Error fetching token symbol (RPC):', error); throw error; }
};

export const getTotalSupply = async (): Promise<bigint> => {
  try { return await coffeeCoinContract.totalSupply(); }
  catch (error) { console.error('[Service] Error fetching total supply (RPC):', error); throw error; }
};

export const getCoffeeCoinBalance = async (userAddress: string): Promise<bigint> => {
  if (!ethers.isAddress(userAddress)) throw new Error('Invalid user address for balance check.');
  try { return await coffeeCoinContract.balanceOf(userAddress); }
  catch (error: any) { console.error(`[Service] Error fetching CFC balance for ${userAddress} (RPC): ${error.message}`); throw error; }
};

export const mintCoffeeCoins = async (recipientAddress: string, amountInWholeTokens: bigint): Promise<string> => {
  if (!serverWalletPrivateKey) throw new Error('Minting service not configured: SERVER_WALLET_PRIVATE_KEY missing.');
  if (!ethers.isAddress(recipientAddress)) throw new Error('Invalid recipient address for minting.');
  if (amountInWholeTokens <= 0n) throw new Error('Mint amount must be positive.');
...
// --- USER TRANSACTION HISTORY FUNCTION (using Etherscan API) ---
export interface FormattedEvent {
  transactionHash: string; blockNumber: number; timestamp?: number;
  type: 'earned' | 'redeemed' | 'sent' | 'received' | 'unknown_transfer';
  amount: string; from: string; to: string; tokenSymbol: string;
}
interface EtherscanTokenTx {
  blockNumber: string; timeStamp: string; hash: string; from: string; to: string;
  value: string; contractAddress: string; tokenName: string; tokenSymbol: string; tokenDecimal: string;
}

export const getCoffeeCoinTransactionHistory = async (userAddress: string): Promise<FormattedEvent[]> => {
  console.log(`[Service TX_HISTORY_ETHERSCAN] For user: ${userAddress}`);
  if (!ETHERSCAN_API_KEY) throw new Error("Etherscan API key not configured for transaction history.");
  if (!coffeeCoinContractAddress) throw new Error("Contract address not configured for transaction history.");
  if (!ethers.isAddress(userAddress)) throw new Error('Invalid user address for history.');
  ...
  try {
    const response = await axios.get(ETHERSCAN_API_URL_SEPOLIA, { params });
    if (response.data.status === "0") {
      if (response.data.message === "No transactions found" || response.data.message === "No records found") return [];
    ...
    const transactions: EtherscanTokenTx[] = response.data.result;
    if (!Array.isArray(transactions)) {
      console.error("[Service TX_HISTORY_ETHERSCAN] Etherscan 'result' not an array:", response.data);
      return [];
    }
    return transactions.map((tx: EtherscanTokenTx) => {
      let type: FormattedEvent['type'];
      ...
      if (txFromLower === ethers.ZeroAddress && txToLower === userAddrLower) type = 'earned';
      else if (txToLower === ethers.ZeroAddress && txFromLower === userAddrLower) type = 'redeemed';
      else if (txToLower === userAddrLower) type = 'received';
      else if (txFromLower === userAddrLower) type = 'sent';
      else {
    ...
    if (axios.isAxiosError(error)) errMsg = `Axios error (tokentx): ${error.message}${error.response ? ` (Status: ${error.response.status})` : ''}`;
...
const ETH_DRIP_AMOUNT_WEI = ethers.parseEther("0.01");
const MIN_ETH_BALANCE_FOR_NO_DRIP_WEI = ethers.parseEther("0.005");
const MAX_TX_COUNT_FOR_DRIP = 5;

interface WalletCheckResult {
  shouldDrip: boolean; ethBalanceWei: bigint; txCount: number; message: string;
}

async function checkWalletForDripEligibility(userAddress: string): Promise<WalletCheckResult> {
  if (!ETHERSCAN_API_KEY) return { shouldDrip: false, ethBalanceWei: 0n, txCount: -1, message: "Etherscan API key missing for drip check." };
  if (!serverWalletPrivateKey) return { shouldDrip: false, ethBalanceWei: 0n, txCount: -1, message: "Server wallet missing for drip." };
  if (!ethers.isAddress(userAddress)) return { shouldDrip: false, ethBalanceWei: 0n, txCount: -1, message: "Invalid user address for drip check." };

  try {
    const balanceParams = { module: 'account', action: 'balance', address: userAddress, tag: 'latest', apikey: ETHERSCAN_API_KEY };
    const balanceRes = await axios.get(ETHERSCAN_API_URL_SEPOLIA, { params: balanceParams });
    if (balanceRes.data.status !== "1") throw new Error(`Etherscan balance check error: ${balanceRes.data.message}`);
    ...
    const txListRes = await axios.get(ETHERSCAN_API_URL_SEPOLIA, { params: txListParams });
    ...
    console.log(`[Service ETH_DRIP_CHECK] User: ${userAddress}, ETH: ${ethers.formatEther(ethBalanceWei)}, TXs: ${txCount}`);
    ...
    let reason = "";
    if (ethBalanceWei >= MIN_ETH_BALANCE_FOR_NO_DRIP_WEI) reason += `Has ${ethers.formatEther(ethBalanceWei)} ETH. `;
    if (txCount >= MAX_TX_COUNT_FOR_DRIP) reason += `Has ${txCount} TXs.`;
...
export const attemptAutoEthDrip = async (recipientAddress: string): Promise<{ dripped: boolean; hash?: string; message: string }> => {
  console.log(`[Service AUTO_ETH_DRIP] Attempting for ${recipientAddress}`);
  if (!serverWalletPrivateKey) return { dripped: false, message: "Faucet (server wallet) not configured." };
  if (!ethers.isAddress(recipientAddress)) return { dripped: false, message: "Invalid recipient address for ETH drip." };
  ...
  const signerWallet = new Wallet(serverWalletPrivateKey, provider);
  try {
    ...
    console.log(`[Service AUTO_ETH_DRIP] Sending ${ethers.formatEther(ETH_DRIP_AMOUNT_WEI)} ETH to ${recipientAddress} from ${signerWallet.address}...`);
    ...
    return { dripped: true, hash: txResponse.hash, message: `Successfully dripped ${ethers.formatEther(ETH_DRIP_AMOUNT_WEI)} ETH. Tx: ${txResponse.hash}` };
  } catch (error: any) {
...
export interface AdminContractOverview {
  ...
  tokenSymbol: string;
  totalMinted?: string;
  totalRedeemedToZeroAddress?: string; // Reflects tokens sent to 0x0 address
  numberOfHolders?: number;
  totalContractTransactions?: number;
}

// Helper to fetch all paginated results from Etherscan
async function fetchAllEtherscanPages(baseUrl: string, initialParams: any, maxPages: number = 5): Promise<any[]> {
  let allResults: any[] = [];
...
export const getAdminContractOverview = async (): Promise<AdminContractOverview> => {
  if (!coffeeCoinContractAddress) {
  ...
  if (!ETHERSCAN_API_KEY) {
    ...
    const [rpcTotalSupply, tokenName, tokenSymbol] = await Promise.all([
        getTotalSupply(), getTokenName(), getTokenSymbol()
    ]);
    return {
  ...
  let creatorInfo: { contractCreator: string; txHash: string } | null = null;
  try {
    ...
    } else console.warn("[AdminService] Could not fetch contract creation from Etherscan:", response.data.message);
  } catch (e: any) { console.warn("[AdminService] Error fetching contract creation from Etherscan:", e.message); }
  ...
  const allTokenTransfers: EtherscanTokenTx[] = await fetchAllEtherscanPages(ETHERSCAN_API_URL_SEPOLIA, tokenTxParams, MAX_PAGES_TOKENTX_OVERVIEW);
  
  const uniqueAddresses = new Set<string>();
  allTokenTransfers.forEach(tx => {
    if (tx.to && tx.to.toLowerCase() !== ethers.ZeroAddress.toLowerCase()) {
...
export const getAdminRecentContractInteractions = async (page: number = 1, offset: number = 10): Promise<EtherscanNormalTransaction[]> => {
  if (!ETHERSCAN_API_KEY) throw new Error("Etherscan API key not configured for recent interactions.");
  if (!coffeeCoinContractAddress) throw new Error("Contract Address missing for recent interactions.");
  if (!CoffeeCoinABI?.abi) throw new Error("CoffeeCoin ABI missing for decoding recent interactions.");
  ...
  try {
    ...
    if (response.data.status === "1" && Array.isArray(response.data.result)) {
      return response.data.result.map((tx: any): EtherscanNormalTransaction => {
